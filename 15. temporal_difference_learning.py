    ...
    def temporal_dofference_learning(self, env, N=30):
        for i in range(5000):
            ...
            while env.possible_actions[state] and n<N:
                ...
                self.value_function[state] = self.get_value_functions(state) + ALPHA*(env.state_reward(new_state) + GAMMA*self.get_value_functions(new_state) - self.get_value_functions(state))
                state = new_state
