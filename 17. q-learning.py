    ...
    def q_learning(self, env, N=300):
        for _ in range(5000):
            ...
            while env.possible_actions[state] and n<N:
                ...
                self.action_value_estimation[state][action] = self.get_action_value_estimation(state, action) + 1/self.state_actions_counter[state][action]*(reward+GAMMA*self.get_action_value_estimation(next_state, next_action) - self.get_action_value_estimation(state, action))
                self.state_actions_counter[state][action] += 1
                state = next_state
